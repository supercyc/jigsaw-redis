...
/**
 * JedisShardTemplate is the JedisTemplate, which has key sharding
 * feature.
 * <p/>
 ...
 * won't do the calculation, so please use JedisShardedTemplate by
 * default.
 * <p/>
...
public class JedisShardedTemplate {
    protected JedisTemplate singleTemplate = null;
    protected ShardingProvider<JedisTemplate> shardingProvider = null;

    public JedisShardedTemplate(JedisPool... jedisPools) {
        this(null, null, jedisPools);
    }
    ...
    public JedisShardedTemplate(ShardingProvider<JedisTemplate> shardingProvider,
            JedisExceptionHandler jedisExceptionHandler, List<JedisPool> jedisPools) {
        this(shardingProvider, jedisExceptionHandler, jedisPools.toArray(new JedisPool[jedisPools.size()]));
    }

    public JedisShardedTemplate(ShardingProvider<JedisTemplate> shardingProvider,
            JedisExceptionHandler jedisExceptionHandler, JedisPool... jedisPools) {
        initShardingProvider(shardingProvider);
        initNodes(jedisExceptionHandler, jedisPools);
    }

    private void initNodes(JedisExceptionHandler jedisExceptionHandler, JedisPool... jedisPools) {
        if (jedisPools.length == 1) {
            singleTemplate = new JedisTemplate(jedisPools[0], jedisExceptionHandler);
            return;
        }

        List<JedisTemplate> jedisTemplates = new ArrayList<JedisTemplate>(jedisPools.length);
        for (JedisPool jedisPool : jedisPools) {
            jedisTemplates.add(new JedisTemplate(jedisPool, jedisExceptionHandler));
        }

        shardingProvider.initNode(jedisTemplates);
    }

    private void initShardingProvider(ShardingProvider<JedisTemplate> shardingProvider) {
        if (shardingProvider == null) {
            this.shardingProvider = new ConsistentHashShardingProvider<JedisTemplate>();
        } else {
    ...
    /**
     * Hash the key and get the jedisTemplate from the hash ring. Get
     * idea from Jedis's Sharded.java
     */
    public JedisTemplate getShard(String key) {
        ...
        return this.shardingProvider.getNode(key);
    }
     ...
     * sharding key.
     */
    public void execute(String shardingKey, PipelineActionNoResult pipelineAction) throws JedisException {
    ...
    public List<String> mget(String shardingKey, String... keys) {
        JedisTemplate jedisTemplate = getShard(shardingKey);
    ...
    public Long lpushx(final String key, final String value) {
        JedisTemplate jedisTemplate = getShard(key);
        return jedisTemplate.lpushx(key, value);
    }

    public Long lpushx(final String key, final String[] values) {
        JedisTemplate jedisTemplate = getShard(key);
        return jedisTemplate.lpushx(key, values);
    }

    public Long lpushx(String shardingKey, final String key, final String value) {
        JedisTemplate jedisTemplate = getShard(shardingKey);
        return jedisTemplate.lpushx(key, value);
    }

    public Long lpushx(String shardingKey, final String key, final String... values) {
        JedisTemplate jedisTemplate = getShard(shardingKey);
        return jedisTemplate.lpushx(key, values);
    }
    ...
    public Long rpush(final String key, final String value) {
        JedisTemplate jedisTemplate = getShard(key);
        return jedisTemplate.rpush(key, value);
    }

    public Long rpush(final String key, final String[] values) {
        JedisTemplate jedisTemplate = getShard(key);
        return jedisTemplate.rpush(key, values);
    }

    public Long rpush(final String shardingKey, final String key, final String value) {
        JedisTemplate jedisTemplate = getShard(shardingKey);
        return jedisTemplate.rpush(key, value);
    }

    public Long rpush(final String shardingKey, final String key, final String[] values) {
        JedisTemplate jedisTemplate = getShard(shardingKey);
        return jedisTemplate.rpush(key, values);
    }
    ...
    public Boolean srem(final String key, final String member) {
        ...
        JedisTemplate jedisTemplate = getShard(key);
        return jedisTemplate.srem(key, member);
    }
