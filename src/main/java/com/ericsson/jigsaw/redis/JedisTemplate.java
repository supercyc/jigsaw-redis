...
import com.ericsson.jigsaw.redis.replication.exception.handler.impl.DefaultJedisExceptionHandler;
import com.ericsson.jigsaw.redis.util.CommandResultProperties;
import com.ericsson.jigsaw.redis.util.InfoCommandResultProperties;

/**
 * JedisTemplate is the template to execute jedis actions, fetch and
 * return the connection from the pool correctly.
 *
 * execute() will throw JigsawJedisException.
 */
public class JedisTemplate {
    ...
    public JedisTemplate(JedisPool jedisPool, JedisExceptionHandler jedisExceptionHandler) {
        this.jedisPool = jedisPool;

        if (jedisExceptionHandler == null) {
            this.jedisExceptionHandler = new DefaultJedisExceptionHandler();
        } else {
    ...
    public boolean equals(Object o) {
        ...
        JedisTemplate that = (JedisTemplate) o;

        if (!jedisPool.equals(that.jedisPool)) {
            return false;
        }
    ...
    @Override
    public int hashCode() {
        return jedisPool.hashCode();
    }

    /**
     * Execute with a call back action with result.
     */
    public <T> T execute(JedisAction<T> jedisAction) throws JigsawJedisException {
        Jedis jedis = null;
        ...
        try {
            ...
            JigsawJedisException exception = new JigsawJedisException(e, jedisPool.getAddress(),
                    jedisPool.getFormattedPoolName());
            broken = exception.isConnectionBroken();
            try {
                jedisExceptionHandler.handleException(exception);
            } catch (Throwable t) {
                logger.error("exception throws during handle exception", t);
            }
     ...
     */
    public void execute(JedisActionNoResult jedisAction) throws JigsawJedisException {
        Jedis jedis = null;
        boolean broken = false;
        try {
            jedis = jedisPool.getResource();
     ...
     */
    public List<Object> execute(PipelineAction pipelineAction) throws JigsawJedisException {
        Jedis jedis = null;
        boolean broken = false;
        try {
            jedis = jedisPool.getResource();
     ...
     */
    public void execute(PipelineActionNoResult pipelineAction) throws JigsawJedisException {
        Jedis jedis = null;
        boolean broken = false;
        try {
            jedis = jedisPool.getResource();
...
//            logger.error(jedisPool.getFormattedPoolName() + "return back jedis failed, will fore close the jedis.", e);
    ...
    /**
     * Remove the specified keys. If a given key does not exist no
     * operation is performed for this key.
     * <p/>
     ...
     * never fails. Set socket timeout to infinite to avoid long time
     * waiting on status code reply when flush on a large DB.
     */
    public String flushDBWithResult() {
        return execute(new JedisAction<String>() {
            ...
            public String action(Jedis jedis) {
                ...
                client.setTimeout(Protocol.DEFAULT_TIMEOUT);
                return result;
            }
        });
    }
     ...
     * null is returned. If the value stored at key is not a string an
    ...
    /**
     * Get the value of the specified key as Long.If the key does not
    ...
    /**
     * Get the value of the specified key as Integer.If the key does
    ...
    /**
     * Get the values of all the specified keys. If one or more keys
     * dont exist or is not of type String, a 'nil' value is returned
     * instead of the value of the specified key, but the operation
     * never fails.
     */
     ...
     * commands: {@link #set(String, String) SET} +
    ...
    /**
     * SETNX works exactly like {@link #setnx(String, String) SET}
     * with the only difference that if the key already exists no
     * operation is performed. SETNX actually means
     * "SET if Not eXists".
     * <p/>
    ...
    /**
     * The command is exactly equivalent to the following group of
     * commands: {@link #setex(String, String, int) SETEX} +
     ...
     * stored at key. The string can't be longer than 1073741824 bytes
     * (1 GB).
     */
     ...
     * exist or contains a value of a wrong type, set the key to the
     * value of "0" before to perform the increment operation.
     * <p/>
     * INCR commands are limited to 64 bit signed integers.
     * <p/>
     * Note: this is actually a string operation, that is, in Redis
     * there are not "integer" types. Simply the string stored at the
     * key is parsed as a base 10 64 bit signed integer, incremented,
     * and then converted back as a string.
     *
     * @return Integer reply, this commands will reply with the new
    ...
    /**
     * return true for add new element, false for only update the
     * score.
     */
    ...
    public ScanResult<String> scan(final String cursor, final String pattern, final Integer count) {
        return execute(new JedisAction<ScanResult>() {
    ...
    public CommandResultProperties info() {
        return execute(new JedisAction<CommandResultProperties>() {
            @Override
            public CommandResultProperties action(Jedis jedis) {
                String infoResult = jedis.info();
                CommandResultProperties pros = new InfoCommandResultProperties();
                pros.loadProperties(infoResult);
                return pros;
            }
        });
    }
